...
template <typename IdType> class RStarTree {
	...
	private:
		...
		std::vector<IdType> get_all_ids(std::vector<IdType>     &ids,
										const RtreeNode<IdType> &node) const;
...
void RStarTree<IdType>::size(int &count, const RtreeNode<IdType> &node) const {
	if (node.type == NodeType::LEAF) {
		...
		for (const RtreeEntry<IdType> &entry : root().entries) {
...
template <typename IdType>
std::vector<IdType> RStarTree<IdType>::get_all_ids(std::vector<IdType>     &ids,
												   const RtreeNode<IdType> &node) const {
