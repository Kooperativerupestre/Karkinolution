...
template <typename IdType> class RStarTree {
  ...
  private:
    ...
    void choose_node_at_depth(std::vector<RtreeNode<IdType>*>& nodes, const Box3D& box,
                              size_t depth);
...
template <typename IdType>
void RStarTree<IdType>::insert_entry(RtreeEntry<IdType> entry, std::vector<RtreeNode<IdType>*> path,
                                     bool already_reinserted) {
...
void RStarTree<IdType>::insert(IdType id, const Box3D& box, bool already_reinserted) {
    ...
    insert_entry(RtreeEntry<IdType>{.box = box, .content = id}, std::move(path),
                 already_reinserted);
}
...
void RStarTree<IdType>::choose_node_at_depth(std::vector<RtreeNode<IdType>*>& nodes,
    ...
    while (nodes.size() - 1 < depth) {
        ...
        if (!node.entries.empty() &&
            std::holds_alternative<std::unique_ptr<RtreeNode<IdType>>>(node.entries[0].content)) {
    ...
    for (const auto& entry : root_->entries) {
        if (Box3DMotor::intersect(box, entry.box)) {
            ...
            if (std::holds_alternative<IdType>(entry.content)) {
                ...
                find(box, std::get<std::unique_ptr<RtreeNode<IdType>>>(entry.content)->entries,
                     ids);
            }
        }
    }
...
template <typename IdType> bool RStarTree<IdType>::remove(IdType id) {
    ...
    if (root_->entries.size() == 1 &&
        std::holds_alternative<std::unique_ptr<RtreeNode<IdType>>>(root_->entries.back().content)) {
        root_ =
