...
struct Components {
    ...
    template <typename T> bool exists() const { return data.contains(std::type_index(typeid(T))); }
    ...
    template <typename T> bool del() { return data.erase(std::type_index(typeid(T))) > 0; }
...
enum class SoilTypes : uint8_t { SAND, ROCK, DIRT, WATER };
