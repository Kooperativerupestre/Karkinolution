...
[[nodiscard]] constexpr Vec3 gravitational_force(const Vec3 &pos1,
												 const Vec3 &pos2,
												 double      mass1,
												 double      mass2,
...
[[nodiscard]] constexpr Vec3 spring_force(const Vec3 &displacement,
										  double      spring_constant) noexcept {
	return displacement * (-spring_constant);
}

[[nodiscard]] constexpr Vec3 damping_force(const Vec3 &velocity,
										   double      damping_coefficient) noexcept {
...
[[nodiscard]] constexpr Vec3
projectile_position(const Vec3 &initial_pos,
					const Vec3 &initial_vel,
					double      time,
					const Vec3 &gravity = Vec3(0.0, -9.80665, 0.0)) noexcept {
	return initial_pos + initial_vel * time + gravity * (0.5 * time * time);
}

[[nodiscard]] constexpr Vec3
projectile_velocity(const Vec3 &initial_vel,
					double      time,
					const Vec3 &gravity = Vec3(0.0, -9.80665, 0.0)) noexcept {
