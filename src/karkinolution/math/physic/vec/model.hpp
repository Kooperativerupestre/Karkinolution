...
public:
  ...
  distance_squared_to(const Vec3 &other) const noexcept {
  ...
  [[nodiscard]] constexpr Vec3 lerp(const Vec3 &other,
                                    double t) const noexcept {
    return Vec3(x + (other.x - x) * t, y + (other.y - y) * t,
                z + (other.z - z) * t);
  }
  ...
  approx_equal(const Vec3 &other, double epsilon = 1e-10) const noexcept {
  ...
  [[nodiscard]] constexpr bool
  is_parallel(const Vec3 &other, double epsilon = 1e-10) const noexcept {
    return cross(other).is_zero(epsilon);
  }

  [[nodiscard]] constexpr bool
  is_perpendicular(const Vec3 &other, double epsilon = 1e-10) const noexcept {
