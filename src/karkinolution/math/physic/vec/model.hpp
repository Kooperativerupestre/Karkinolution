...
class Vec3 {
  public:
    ...
    [[nodiscard]] constexpr bool approx_equal(const Vec3& other,
                                              double epsilon = 1e-10) const noexcept {
    ...
    [[nodiscard]] constexpr bool is_parallel(const Vec3& other,
                                             double epsilon = 1e-10) const noexcept {
        return cross(other).is_zero(epsilon);
    }

    [[nodiscard]] constexpr bool is_perpendicular(const Vec3& other,
                                                  double epsilon = 1e-10) const noexcept {
    ...
    [[nodiscard]] constexpr double max_component() const noexcept { return std::max({x, y, z}); }

    [[nodiscard]] constexpr double min_component() const noexcept { return std::min({x, y, z}); }
...
constexpr Vec3 operator*(double scalar, const Vec3& vec) noexcept { return vec * scalar; }
