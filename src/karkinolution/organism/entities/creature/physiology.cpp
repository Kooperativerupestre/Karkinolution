...
float CreatureGrowingPhysiology::get_new_max_energy_increment(const Creature& creature, const EntitiesRegistry&entities) {
    ...
    const NormalizedValue<float> remaining =
        1.0f - metabolism.energy.max() / max_energy;

    const auto& conversion =
        metabolism_genome.transformations
            .at(Resource::RESERVED_ENERGY, MetabolismTrait::MAX_ENERGY)
            .efficiency;
...
float CreatureGrowingPhysiology::get_new_max_energy_reserved_increment(const Creature& creature, const EntitiesRegistry&entities) {
    ...
    const float max_reserved_energy =
        metabolism_genome.max_reserved_energy;

    const NormalizedValue<float> remaining =
        1.0f - metabolism.reserved.max() / max_reserved_energy;
...
float CreatureGrowingPhysiology::get_new_max_life_increment(const Creature& creature, const EntitiesRegistry&entities) {
    ...
    const NormalizedValue<float> remaining =
        1.0f - body.vital.life.max() / max_life;

    const auto& conversion =
        vital_genome.transformations
            .at(Resource::RESERVED_ENERGY, VitalTrait::MAX_LIFE)
            .efficiency;

    const GrowthRate growth =
        vital_genome.growth_rates.life.value;
...
float CreatureGrowingPhysiology::get_new_lateral_increment(const Creature &creature, const EntitiesRegistry&entities) {
    ...
    const NormalizedValue<float> remaining = 1.0f - body.morphology.size.lateral.value/morphology_genome.average_lateral.value;
    const Factor<float, 2.0f, 0.0f> muscle_factor{
        creature.specie_relative_muscle()
    };
...
float CreatureGrowingPhysiology::get_new_back_increment(const Creature &creature, const EntitiesRegistry&entities) {
    ...
    const NormalizedValue<float> remaining = 1.0f - body.morphology.size.back.value/morphology_genome.average_back.value;
    const Factor<float, 2.0f, 0.0f> muscle_factor{
        creature.specie_relative_muscle()
    };
...
float CreatureGrowingPhysiology::get_new_height_increment(const Creature &creature, const EntitiesRegistry&entities) {
    ...
    const NormalizedValue<float> remaining = 1.0f - body.morphology.size.height.value/morphology_genome.average_back.value;
    const Factor<float, 2.0f, 0.0f> muscle_factor{
        creature.specie_relative_muscle()
    };
...
CreatureFunction CreaturePhysiology::get_diseases_effect(const Creature& creature, const EntitiesRegistry& entities) {
    return {};
}