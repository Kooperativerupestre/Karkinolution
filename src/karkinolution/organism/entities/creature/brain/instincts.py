...
#
#
# This file is here because ths system is good, and I'm copying this for now until i have better ideas
#
#
...
class AttackResolver:
    ...
    @staticmethod
    def get_reactive_attack(
        creature: Creature, entities: EntitiesRegistry
    ) -> AttackOutput | None:
        if (
            creature.last_attack is not None
            and creature.last_attack.attacker_id in entities.entitys
        ):
            target_id = creature.last_attack.attacker_id
            score = AttackResolver.score_attack(
                creature, entities.get_creature(target_id)
            )

            return AttackOutput(score, target_id)

    @staticmethod
    def resolve_predicate_attack(
        creature: Creature,
        radius: Coord,
        perception: Perception,
        predicate: Callable[[PerceivedBlock], bool],
    ) -> Coord | None:
    ...
    @staticmethod
    def resolve_aggressive_attack(
        creature: Creature, perception: Perception, entities: EntitiesRegistry
    ) -> AttackOutput | None:
        target_coord = AttackResolver.resolve_predicate_attack(
            creature,
            Coord(3, 2),
            perception,
            lambda b: b.entity.specie_id == creature.genome.core.id,
        )  # type: ignore
    ...
    @staticmethod
    def resolve_territorial_attack(
        creature: Creature, perception: Perception, entities: EntitiesRegistry
    ) -> AttackOutput | None:
        target_coord = AttackResolver.resolve_predicate_attack(
            creature, Coord(2, 2), perception, lambda b: True
        )
        if target_coord is None:
            return None
        target_id = perception.get(target_coord).entity.identity  # type: ignore
    ...
    def resolve_attack(
        ...
        if attack_output is None:
            if temperament == Temperament.AGGRESSIVE:
                attack_output = AttackResolver.resolve_aggressive_attack(
                    creature, perception, entities
                )
            elif temperament == Temperament.TERRITORIAL:
                attack_output = AttackResolver.resolve_territorial_attack(
                    creature, perception, entities
                )
...
class Instincts:
    @staticmethod
    def take(
        creature: Creature, reproductive_buffer: ReproductiveBuffer
    ) -> list[IntentScored]:
        acts: list[IntentScored] = []
        # FIND_FOOD
        acts.append(
            IntentScored(IntentActs.FIND_FOOD, ScorerIntents.score_find_food(creature))
        )
        # NOTHING
        acts.append(
            IntentScored(IntentActs.NOTHING, ScorerIntents.score_nothing(creature))
        )
        # FIND MATCH
        if (
            creature.reproductively_capable
            and reproductive_buffer.require(creature.id) is None
        ):
            acts.append(
                IntentScored(
                    IntentActs.FIND_MATCH, ScorerIntents.score_find_match(creature)
                )
            )
        return acts
...
def try_call_reproductive_buffer(
    intent: Intent, creature: Creature, reproductive_buffer: ReproductiveBuffer
) -> None:
    if intent.intent == IntentActs.FIND_MATCH:
        reproductive_buffer.registry(
            ReproductiveDesire(creature.id, creature.genome.core.id)
        )
...
class PlannerFindFood:
    @staticmethod
    def decide_preset(
        food_target: FoodOption, perception: Perception
    ) -> MovePreset | EatPreset | AttackPreset:
        # ALIAS
    ...
    @staticmethod
    def plan_intent(
        perception: Perception, creature: Creature
    ) -> MovePreset | EatPreset | AttackPreset | None:
...
class PlannerFindMatch:
    ...
    def plan_intent(perception: Perception, creature: Creature) -> MovePreset | None:
        same_specie = Analysis.find_predicate(
            perception,
            predicate=lambda b: (
                b.get_entity_type() == EntityTypes.CREATURE
                and ReproductiveSystem.can_reproduce(creature, b.entity)
            ),
        )  # type: ignore
...
class PlannerAttack:
    @staticmethod
    def plan_intent(
        attack_output: AttackOutput, perception: Perception, entities: EntitiesRegistry
    ) -> MovePreset | AttackPreset:
...
class PlannerNothing:
    ...
    def score_coord(perception: Perception, coord: Coord, creature: Creature) -> float:
        ...
        has_same_specie = (
            block.get_entity_type() == EntityTypes.CREATURE
            and block.entity.specie_id == creature.genome.core.id
        )
        ...
        factors = FactorsCalc.get_territorial_social_indifference_factor(
            creature
        ) * FactorsCalc.get_pregnant_social_interest_factor(creature)
    ...
    def plan_intent(perception: Perception, creature: Creature) -> MovePreset | None:
        ...
        for c in perception.iter_keys:
            if (
                c != creature.position
                and c.distance_to_other(creature.position)
                < perception.max_distance * 0.7
            ):
                coords_s.append(
                    PlannerNothing.CoordScored(
                        c,
                        PlannerNothing.score_coord(perception, c, creature)
                        * uniform(1, 1 + GLOBAL_CONFIG.noise_nothing_intent),
                    )
                )
...
class Planner:
    ...
    @staticmethod
    def plan(
        perception: Perception, creature: Creature, entities: EntitiesRegistry
    ) -> MovePreset | EatPreset | AttackPreset | None:
