...
CreatureSkeletonGenome GenomeMotor::scramble(const CreatureSkeletonGenome &g1,
											 const CreatureSkeletonGenome &g2) {
	auto new_transformations = GeneticDisturbs::mix_transformations(
		g1.transformations,
		g2.transformations,
		[](const Efficiency &v1, const Efficiency &v2) {
			return Efficiency{GeneticDisturbs::scramble(
				v1.value(),
				v2.value(),
				GeneticDisturbs::StandardContexts::transformations_scramble)};
		});

	return CreatureSkeletonGenome{
		.shared_volume =
			GeneticDisturbs::scramble(g1.shared_volume.value(),
									  g2.shared_volume.value(),
									  GeneticDisturbs::StandardContexts::morphology_scramble),
		.average_bones =
			GeneticDisturbs::scramble(g1.average_bones.value,
		...
		.growth =
			GeneticDisturbs::scramble(g1.growth.value,
									  g2.growth.value,
									  GeneticDisturbs::StandardContexts::metabolism_growth_rates),
		.transformations = new_transformations};
}

CreatureMorphologyGenome GenomeMotor::scramble(const CreatureMorphologyGenome &g1,
	...
	return Genomes::CreatureGenomes::Morphology::MorphologyGenome{
		.average_lateral =
			GeneticDisturbs::scramble(g1.average_lateral.value,
									  g2.average_lateral.value,
									  GeneticDisturbs::StandardContexts::morphology_scramble),
		.average_back =
			GeneticDisturbs::scramble(g1.average_back.value,
									  g2.average_back.value,
									  GeneticDisturbs::StandardContexts::morphology_scramble),
		.average_height = GeneticDisturbs::scramble(
			g1.average_height.value,
			g2.average_height.value,
			ScrambleContext{.max_base            = 0.70f,
							.min_base            = 1.30f,
							.distance_multiplier = 1.5f,
							.mutations_counts    = 2,
							.suppress_context =
								SuppressContext{.chance_of_v1_being_suppressed = 0.4f,
		...
		.average_mass = GeneticDisturbs::scramble(
			...
			ScrambleContext{.max_base            = GeneticDisturbs::LOW_SCRAMBLE_MAX_BASE,
							...
							.suppress_context =
								SuppressContext{.chance_of_v1_being_suppressed = 0.25f,
...
Diet GenomeMotor::scramble(const Diet &d1, const Diet &d2) {
	auto new_diet = Diet{// scramble: mixes two values with randomness. Parameters:
...
Genomes::CreatureGenomes::Metabolism::MetabolismGenome
GenomeMotor::scramble(const Genomes::CreatureGenomes::Metabolism::MetabolismGenome &g1,
	...
	auto new_growth_rates = MetabolismGrowthRates{
		.max_energy = GeneticDisturbs::scramble(g1.growth_rates.max_energy.value,
		...
		.max_reserved_energy =
			GeneticDisturbs::scramble(g1.growth_rates.max_reserved_energy.value,
	...
	return Genomes::CreatureGenomes::Metabolism::MetabolismGenome{
		.max_energy = GeneticDisturbs::scramble(
			g1.max_energy,
			g2.max_energy,
			ScrambleContext{.max_base = 0.80f, .min_base = 1.20f, .distance_multiplier = 0.2f}),
		.max_reserved_energy = GeneticDisturbs::scramble(
			g1.max_reserved_energy,
			g2.max_reserved_energy,
			ScrambleContext{.max_base = 0.91f, .min_base = 1.09f, .distance_multiplier = 0.1f}),
		.diet            = scramble(g1.diet, g2.diet),
		.transformations = new_transformations,
		.growth_rates    = new_growth_rates};
}

ReproductionViviparous GenomeMotor::scramble(const ReproductionViviparous &g1,
											 const ReproductionViviparous &g2) {
	return {};
}

Genomes::CreatureGenomes::Reproduction::OviparousOrganismGenome
GenomeMotor::scramble(const Genomes::CreatureGenomes::Reproduction::OviparousOrganismGenome &g1,
					  const Genomes::CreatureGenomes::Reproduction::OviparousOrganismGenome &g2) {
	return {};
}

ReproductiveGenome GenomeMotor::scramble(const ReproductiveGenome &g1,
										 const ReproductiveGenome &g2) {
	const auto children = std::max(
		1,
		static_cast<int>(std::lround(GeneticDisturbs::scramble(
	...
	const int fertility_limit = std::max(
		1,
		static_cast<int>(std::lround(GeneticDisturbs::scramble(
			g1.fertility_limit,
			g2.fertility_limit,
	...
	const float reproduction_cost = GeneticDisturbs::scramble(
		...
		ScrambleContext{.max_base            = GeneticDisturbs::LOW_SCRAMBLE_MAX_BASE,
						.min_base            = GeneticDisturbs::LOW_SCRAMBLE_MIN_BASE,
						.distance_multiplier = GeneticDisturbs::LOW_DISTANCE_MULTIPLIER});
	const int average_gestation_limit = std::max(
		1,
		static_cast<int>(std::lround(GeneticDisturbs::scramble(
	...
	if (g1.reproductive_way() == ReproductiveWays::OVIPAROUS) {
		new_way_genome =
			GenomeMotor::scramble(std::get<ReproductionOviparous>(g1.reproductive_way_genome),
								  std::get<ReproductionOviparous>(g1.reproductive_way_genome));
	} else { // reproductive_way == VIVIPAROUS
		new_way_genome =
			GenomeMotor::scramble(std::get<ReproductionViviparous>(g1.reproductive_way_genome),
								  std::get<ReproductionViviparous>(g2.reproductive_way_genome));
	}

	return {.reproductive_way_genome = new_way_genome,
			.average_children_count  = children,
			.average_gestation_limit = average_gestation_limit,
			.fertility_limit         = fertility_limit,
			.reproduction_cost       = reproduction_cost};
...
GenomeMotor::scramble(const Genomes::CreatureGenomes::Vital::VitalGenome &g1,
...
BrainGenome GenomeMotor::scramble(const BrainGenome &g1, const BrainGenome &g2) {
	return BrainGenome{.average_sociability =
						   GeneticDisturbs::scramble(g1.average_sociability.value(),
													 g2.average_sociability.value(),
													 ScrambleContext{.max_base            = 1.30f,
																	 .min_base            = 0.70f,
...
GenomeMotor::scramble(const Genomes::EmbryoGenomes::BodyGenome &g1,
	...
	const ScrambleContext body_context{
		...
		.distance_multiplier = GeneticDisturbs::LOW_DISTANCE_MULTIPLIER,
		.mutations_counts    = 1,
		.suppress_context    = SuppressContext{.chance_of_v1_being_suppressed = 0.20f,
											   .chance_of_v2_being_suppressed = 0.20f,
											   .suppressed_multiplier         = 0.40f}};

	return EmbryoBodyGenome{
		.average_max_energy = GeneticDisturbs::scramble(g1.average_max_energy.value(),
														g2.average_max_energy.value(),
														body_context),
		.average_max_life   = GeneticDisturbs::scramble(g1.average_max_life.value(),
                                                      g2.average_max_life.value(),
                                                      body_context),
		.average_health     = GeneticDisturbs::scramble(g1.average_health.value(),
                                                    g2.average_health.value(),
                                                    body_context)};
}

Genomes::EmbryoGenomes::Morphology
GenomeMotor::scramble(const Genomes::EmbryoGenomes::Morphology &g1,
	...
	return EmbryoMorphologyGenome{
		.volume_growth = GeneticDisturbs::scramble(
			...
			ScrambleContext{.max_base            = 1.15f,
							...
							.mutations_counts    = 1,
							.suppress_context =
								SuppressContext{.chance_of_v1_being_suppressed = 0.18f,
												.chance_of_v2_being_suppressed = 0.18f,
												.suppressed_multiplier         = 0.50f}}),
		.average_volume =
			GeneticDisturbs::scramble(g1.average_volume.value(),
