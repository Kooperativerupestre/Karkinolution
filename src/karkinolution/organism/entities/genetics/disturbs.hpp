...
namespace GeneticDisturbs {
    // Scramble base values - use either max_base or min_base, not both, to avoid inconsistent behavior (e.g., 1.05f with 0.70f)
    ...
    struct ScrambleContext {
        ...
        SuppressContext suppress_context = SuppressContext{
            .chance_of_v1_being_suppressed = 0.28f,
    ...
    struct TimeScrambleContext {
        T max_base, min_base;
        int mutation_counts;
    };

    namespace StandardContexts {
    inline constexpr auto transformations_scramble = ScrambleContext{
        .max_base = 1.10f,
        .min_base = 0.90f,
    ...
    inline constexpr auto morphology_scramble = ScrambleContext{
        .max_base = 1.15f,
        .min_base = 0.85f,
        .distance_multiplier = 0.4f,
        .inheritance_bias = 1.0f/2.0f,
    ...
    inline constexpr auto metabolism_growth_rates = ScrambleContext{
        .max_base = 0.97f,
        .min_base = 1.03f,
        .distance_multiplier = 0.6f
    };
    }
    ...
    float scramble(T e_v1, U e_v2, const ScrambleContext&context) {
        ...
        float value = inheritance_factor * RandomGenerators::generate(context.min_base, context.max_base) + RandomGenerators::generate(-distance, distance);
    ...
    template <std::integral T>
    T time_scramble(T v1, T v2, const TimeScrambleContext<T>&context) {
        T value = 0;
...
requires std::invocable<F, Efficiency, Efficiency> &&
         std::same_as<
             std::invoke_result_t<F, Efficiency, Efficiency>,
             Efficiency
         >
Transformations<Trait> mix_transformations(
    const Transformations<Trait>& lhs,
        ...
        for (const auto& lhs_relation : lhs_relations) {
            ...
            if (rhs_it == rhs_relations.end()) {
                throw std::out_of_range(
                    "Matching transformation not found"
                );
            }

            Relation<Trait> relation = lhs_relation;

            relation.efficiency = std::invoke(
