...
namespace GeneticDisturbs {
	// Scramble base values - use either max_base or min_base, not both, to avoid inconsistent
	// behavior (e.g., 1.05f with 0.70f)
	...
	struct ScrambleContext {
			...
			SuppressContext suppress_context =
				SuppressContext{.chance_of_v1_being_suppressed = 0.28f,
	...
	Transformations<Trait> mix_transformations(const Transformations<Trait> &lhs,
		...
		for (const auto &[resource, lhs_relations] : lhs.view()) {
			...
			for (const auto &lhs_relation : lhs_relations) {
				const auto rhs_it =
					std::find_if(rhs_relations.begin(),
								 rhs_relations.end(),
				...
				relation.efficiency =
					std::invoke(function, lhs_relation.efficiency, rhs_it->efficiency);
