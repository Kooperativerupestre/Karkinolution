...
void UterusMotor::transfer_energy_to_uterus(Creature &creature, float energy, float reserved) {
    ...
    const float uterus_remaining =
        uterus.energy.max() - uterus.energy.value();

    const float real_energy_needed =
        std::min(uterus_remaining, energy + reserved);

    const float energy_used =
        std::min(energy, real_energy_needed);

    const float reserved_used =
        real_energy_needed - energy_used;
    uterus.energy += real_energy_needed;
    creature.body.metabolism.energy -= energy_used;
    creature.body.metabolism.reserved -= reserved;
}

void UterusMotor::transfer_energy_to_uterus(Creature&creature, NormalizedValue<float>energy_ratio, NormalizedValue<float> reserved_ratio) {
    transfer_energy_to_uterus(creature, energy_ratio.value() * creature.body.metabolism.energy.value(), reserved_ratio.value() * creature.body.metabolism.reserved.value());
}
