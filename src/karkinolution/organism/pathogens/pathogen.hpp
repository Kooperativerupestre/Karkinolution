...
template <typename T>
concept HasId = requires(T obj) { obj.id; };

template <typename T>
concept HasHealth =
	requires(T obj) { requires std::same_as<decltype(obj.health), OrganismStats::Health::Health>; };

template <HasIdSpecie GenericPathogen, HasId PathogenId> class PathogenContainer {
	protected:
	...
	public:
		...
		GenericPathogen &at(PathogenId id) {
			...
			throw IdNotFoundError(std::format("ID {} was not found", id.id));
		}
...
template <HasIdSpecie GenericPathogen, HasId PathogenId, typename Specie> class PathogenImages {
	private:
	...
	public:
		...
		PathogenImages(PathogenContainer<GenericPathogen, PathogenId> &&container) {
			_pathogens.reserve(container.size());

			for (auto &pathogen : container.virus()) {
				_pathogens.emplace(pathogen.id.specie, std::move(pathogen));
			}
		}
