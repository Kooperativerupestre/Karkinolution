...
[[nodiscard]] ViralLoad
VirusPhysiology::calculate_min_viral_load(const V                        &virus,
...
template <VirusType V>
[[nodiscard]] ViralLoad
VirusPhysiology::calculate_max_viral_load(const V                        &virus,
										  const MetamorphosisEnvironment &environment) {
	return virus.viral_load.value() + 0.5f
		- environment.efficiency.value() * environment.immunity.value();
}

[[nodiscard]] Vorax VirusPhysiology::metamorphosis(const Vorax                    &virus,
												   const MetamorphosisEnvironment &environment) {
	...
	return Vorax{.id                 = VirusIDF::create_virus_id(VirusSpecies::VORAX),
				 .viral_load         = ViralLoad{RandomGenerators::generate(min_viral_load.value(),
                                                                    max_viral_load.value())},
...
[[nodiscard]] MorbusExiguus
VirusPhysiology::metamorphosis(const MorbusExiguus            &virus,
							   const MetamorphosisEnvironment &environment) {
	const auto min_viral_load = calculate_min_viral_load(virus, environment);
	const auto max_viral_load = calculate_max_viral_load(virus, environment);
	...
	return MorbusExiguus{
		...
		.viral_load =
			ViralLoad{RandomGenerators::generate(min_viral_load.value(), max_viral_load.value())},
