...
class Approx {
public:
    constexpr explicit Approx(
        T value,
        T epsilon_abs = std::numeric_limits<T>::epsilon() * 10,
        T epsilon_rel = std::numeric_limits<T>::epsilon() * 10
    )
        : value_(value), epsilon_abs_(epsilon_abs), epsilon_rel_(epsilon_rel) {
        assert(std::isfinite(value_) &&
               "Approx: Nan or infinite value received");
    }
    ...
    constexpr friend bool operator==(const Approx& lhs, T rhs) { return rhs == lhs; }

    constexpr friend bool operator!=(T lhs, const Approx& rhs) { return !(lhs == rhs); }
    constexpr friend bool operator!=(const Approx& lhs, T rhs) { return !(rhs == lhs); }
