...
namespace Choices {
	...
	template <typename T>
	bool choice_bool(
		const NormalizedValue<float>& true_weight,
		const NormalizedValue<float>& v2
		) {
		return Choices::choices(
			std::array<bool, 2>{true, false},
...
namespace RandomGenerators {
	...
	template <typename T, typename U>
	SupressOneOutput suppress_one(
		T& v1,
		U& v2,
		const SuppressContext& context
		) {
	...
	template <typename T, typename U>
	SuppressTwoOutput suppress_two(
		T& v1,
		U& v2,
		const SuppressContext& context
		) {
		bool v1_has_suppressed =
			Choices::choice_bool(context.chance_of_v1_being_suppressed);
		bool v2_has_suppressed =
			Choices::choice_bool(context.chance_of_v2_being_suppressed);
