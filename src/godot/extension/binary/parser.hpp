...
inline godot::Ref<GodotCreature> parse_creature(const godot::PackedByteArray &payload,
										   std::uint64_t                id = 0) {
	return GodotCreature::from_deserialized(CreatureResponseDSI::get_creature(to_bytes(payload)), id);
}

inline godot::PackedByteArray build_get_creature_request(std::uint64_t id) {
	return to_packed_byte_array(
		FrameMotor::build(BinarySubTypes::Request::GET_CREATURE, Serializer::convert_uint64_t(id)));
}
